...
function DropableColumn({
  ...
  return (
    <Card className="min-w-75 shrink-0 shadow-md p-0">
      ...
      <CardContent
        ref={setNodeRef}
        className={`space-y-2  pt-4 bg-gray-50/50 min-h-100 roundedb-lg${isOver ? " ring-2 ring-blue-500" : ""}`}
      >
        <SortableContext
...
export default function KanbanBoard({ board, userId }: KanbanBoardProps) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    //check if dropped on  a column or  another job
    ...
    if (targetColumn) {
      ...
      if (targetIndexInFiltered !== -1) {
        ...
        newOrder = allJobsInTargetInFiltered.length;
      }
    } else {
      return;
    }
